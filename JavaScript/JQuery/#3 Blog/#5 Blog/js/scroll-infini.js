        ...
        $.ajax({
            ...
            success: function(response) { // L'argument "response", contiendra les données retournées
                ...
                for (pageNext; pageNext < pageMax; pageNext++) {
                    ...
                    // Si "article" ne possède plus de valeur, on affiche un message
                    if (article === undefined) {

                        // Si l'élément "p" au dessus n'existe pas, on le créer
                        if ($(".btn").parent().prev("p.text-warning").length == 0) {
                            $(".btn").parent().before('<p class="text-warning text-center">Vous êtes à la fin !</p>');
                        }

                        // Stoppe la boucle là où elle est.
                        // Même fonctionnement que dans un switch().
                        break;
                    }
            ...
            error: function(error) { // L'argument "error", contiendra l'erreur retournée
                ...
                // Si l'élément "p" au dessus n'existe pas, on le créer, sinon on le met à jour
                if ($(".btn").parent().prev("p.text-danger").length == 0) {
                    $(".btn").parent().before(`<p class="text-danger text-center">${message}</p>`);
                }
                else {
                    $("p.text-danger").text(message);
                }
